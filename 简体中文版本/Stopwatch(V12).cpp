...
void file()
{
	...
	if(c=='1'){
		cout<<"请输入文件路径：";
	while(strlen(tr)<2)gets(tr);
	}
	if(c=='1'){int oldstdout = dup(STDOUT);
freopen(tr,"w",stdout);
	SYSTEMTIME sys;
	GetLocalTime( &sys );
	printf("Time When Saved:%d年%d月%d日%d时%d分%d秒%d毫秒\n",sys.wYear,sys.wMonth,sys.wDay,sys.wHour,sys.wMinute,sys.wSecond,sys.wMilliseconds);
	...
	}freopen("CON","w",stdout);dup2(oldstdout,STDOUT);
	}
	/*if(c=='2'){
	ifstream fin(tr);
...
//	scanf("The number of data:%d\n",&ret);
	...
	if(c=='2'){int oldstdout = dup(STDOUT);
	freopen("StopwatchData.db","w",stdout);
	cout<<ynply<<endl<<RunTimeStop<<endl<<refresh<<endl<<theme<<endl<<k<<endl<<jdt<<endl<<jdtl;/*save(tr);*/
	freopen("CON","w",stdout);dup2(oldstdout,STDOUT);
	}
	if(c=='3'){
...
void exterior()
{
	...
	if(c=='3'){
		...
		c=getch();
		int n;
		if(!theme&&c=='1')n=MessageBox(NULL,"用不正常样式可能会出现问题，确定要用吗？","提示",MB_YESNO|MB_ICONQUESTION);
		if(c=='1'&&n==IDYES&&!theme){theme=1;cout<<"大小(建议<150;最大32767,否则会出现错误)：";cin>>k;return;}
		if(c=='1'&&theme)theme=0;
	}
	if(c=='4'){
		cout<<"按1";
		if(jdt)cout<<"隐藏";
		...
		int n;
		c=getch();
		if(!jdt&&c=='1')n=MessageBox(NULL,"使用进度条可能会出现问题，确定要用吗？","提示",MB_OKCANCEL|MB_ICONQUESTION);
		if(c=='1'&&n==IDOK&&!jdt){jdt=1;cout<<"长度(建议<500;最大32767,否则会出现错误)：";cin>>jdtl;return;}
		if(c=='1'&&jdt)jdt=0;
	}
}
void settings()
...
void pause()
{
	...
	cout<<"\n2\t重新开始\n3\t设置\n4\t文件\n5\t退出\n其他键\t继续\n";
	char c=getch();
	if(c=='1')hour=yh,minute=ym,second=ys,millis=0,ret=0;
	if(c=='2')ynply=1;
	if(c=='3')settings();
	if(c=='4')file();
	if(c=='5')exitt();
...
int main()
{
	...
	cout<<"欢迎使用计时器\nVersion 11\n由zym制作\n版权所有，侵权必究";
	MessageBox(NULL,"如果在设置时间时输入非数字的字符，会出现错误，后果自负。","警告",MB_OK|MB_ICONWARNING);
	reply:ret=0;
	system("cls");cout<<"你想计时还是倒计时？"<<endl<<"0:秒表 1:倒计时"<<endl;
	cin>>n;
	while(n!=0&&n!=1)
	{
	...
	if(n==0){
		...
		while(1)
		{
			now=clock()/10;
			millis-=now-last;
			if(millis<0)second+=millis/100-1,millis=100+millis%100;
			...
			system("cls");
			if(jdt){
				cout<<' ';for(i=1;i<=jdtl;i++)cout<<'-';cout<<'\n';cout<<'|';
				for(i=1;i<=jdtl;i++)if(i<=(double)(hour*3600+minute*60+second)/(yh*3600+ym*60+ys)*jdtl+1)cout<<'#';
				else cout<<' ';
				printf("|%lf%%\n ",(double)(hour*3600+minute*60+second)/(yh*3600+ym*60+ys)*100);
				for(i=1;i<=jdtl;i++)cout<<'-';cout<<" \n";
			}
			if(theme==0)printf("%02d:%02d:%02d.%02d\n",hour,minute,second,millis);
			else if(theme=1)ec(hour,minute,second,millis);
			if(ret==0){
