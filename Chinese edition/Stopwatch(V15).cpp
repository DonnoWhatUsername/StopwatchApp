...
//Terrible experience: Don't save in Visual Studio Code!
#include <iostream>
#include <cstdio>
#include <fstream>
#include <Windows.h>
...
int refresh = 1000;
string list[5][7] = {"秒表", "倒计时", "\0", "\0", "\0", "\0", "\0",											 //start
					 "继续", "复位", "重新开始", "设置", "文件", "重启此应用", "退出",							 //pause
					 "继续", "外观", "屏幕刷新频率", "后台刷新频率", "显示菜单时是否停止计时", "清空记录", "\0", //settings
					 "继续", "保存数据", "保存设置（除颜色及窗口大小）", "载入设置", "\0", "\0", "\0",			 //files
					 "继续", "颜色", "窗口大小", "计时器样式", "进度条（设置仅对倒计时起作用）"};				 //appearance
struct a
{
	short hour, minu, sec, mil;
} record[101];
void ccolor(bool tocha)
{
	if (tocha)
...
short choice(int block, int maxlist, bool display, string words)
{
	short usrc = 0;
	maxlist--;
	outputlist(block, maxlist, 0, display, words);
	char c = '`';
	while (c != ' ')
	{
		if (c == 'w' || c == 72 || c == 77)
...
void file()
{
	short c = choice(3, 4, 0, "Tip:导入数据时请不要更改原输出数据\n");
	char tr[303] = {' '};
	if (c == 1)
	{
		cout << "请输入文件路径：";
...
void appearance()
{
	short c = choice(4, 5, 0, "\0");
	if (c == 1)
	{
		cout << "暂不支持此功能，请使用 Version 14 或 Version 13。\n";
		system("pause");
	}
	if (c == 2)
	{
...
{
	short i;
	system("title Stopwatch && color 0f");
	cout << "欢迎使用计时器\nVersion 13\n由zym制作\n版权所有，侵权必究";
